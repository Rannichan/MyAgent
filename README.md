...
默认共享用户设定保存在 `agent/user.md`。
Agent 配置保存在 `agent/profiles/<id>/` 下，包含以下文件：
...
后端会按固定顺序拼接为 system prompt：`agent -> identity -> user -> soul -> memory`（每段带对应标题）。

同时支持在前端顶部「Agent 管理」中新增、编辑、删除并保存 Agent 配置（保存为 `agent/profiles/<id>/agent.md`、`identity.md`、`memory.md`、`soul.md`），并在 Agent 模式下选择不同 Agent。
